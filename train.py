...
def reset_cfg(cfg, args):
    ...
    if args.domains:
        ...
        if cfg.DATASET.NAME == "OfficeHome":
            # cfg.TRAINER.PMCC.STRONG_TRANSFORMS = ["random_flip", "randaugment", "normalize"]
            ...
            # cfg.TRAINER.PMCC.STRONG_TRANSFORMS = ["randaugment", "normalize"]
...
def extend_cfg(cfg, args):
    ...
    if args.trainer == 'CLIP_ZS' or args.trainer == 'CLIP_LR' or args.trainer == 'CLIP_FC' or args.trainer == 'CLIP_FT':
        ...
        cfg.TRAINER.PDA = CN()
        cfg.TRAINER.PDA.PREC = "fp16"
        cfg.TRAINER.PDA.DROPOUT = 0.0
        
        cfg.TRAINER.PDA.TP = True
        cfg.TRAINER.PDA.T_DEEP = False
        cfg.TRAINER.PDA.N_CTX = 2
        cfg.TRAINER.PDA.CSC = False
        cfg.TRAINER.PDA.CTX_INIT = "a photo of a"
        cfg.TRAINER.PDA.CLASS_TOKEN_POSITION = "end"
        
        cfg.TRAINER.PDA.VP = False
        cfg.TRAINER.PDA.V_DEEP = False
        cfg.TRAINER.PDA.NUM_TOKENS = 2
        cfg.TRAINER.PDA.DEEP_SHARED = False
        cfg.TRAINER.PDA.DEEP_LAYERS = None
        cfg.TRAINER.PDA.SHARE_LAYER = [0, 5]
        cfg.TRAINER.PDA.LOCATION = "middle"

    elif args.trainer == 'PMCC':
        cfg.TRAINER.PMCC = CN()
        cfg.TRAINER.PMCC.PREC = "fp16"  # fp16, fp32, amp
        cfg.TRAINER.PMCC.DROPOUT = 0.0

        cfg.TRAINER.PMCC.TP = True
        cfg.TRAINER.PMCC.T_DEEP = False
        cfg.TRAINER.PMCC.N_CTX = 2  # number of text context vectors
        cfg.TRAINER.PMCC.CSC = False  # class-specific context
        cfg.TRAINER.PMCC.CTX_INIT = "a photo of a"  # initialization words
        cfg.TRAINER.PMCC.CLASS_TOKEN_POSITION = "end"  # 'middle' or 'end' or 'front'

        cfg.TRAINER.PMCC.VP = False
        cfg.TRAINER.PMCC.V_DEEP = False
        cfg.TRAINER.PMCC.NUM_TOKENS = 2  # number of visual context vectors
        cfg.TRAINER.PMCC.DEEP_SHARED = False  # whether relation or not
        cfg.TRAINER.PMCC.DEEP_LAYERS = None  # if set to be an int, then do partial-deep prompt tuning
        cfg.TRAINER.PMCC.SHARE_LAYER = [0, 5]  # the prompt of front 5 layer is shared
        cfg.TRAINER.PMCC.LOCATION = "middle"

...
class PromptLearner(Base_PromptLearner):
    def __init__(self, cfg, classnames, clip_model):
        ...
        n_cls = len(classnames)                                     # office31: 31
        # print("Number of classes: ", n_cls)
        n_ctx = cfg.TRAINER.PDA.N_CTX                               # From command line input
        # print("Number of Context prompt: ", n_ctx)
        ctx_init = cfg.TRAINER.PDA.CTX_INIT                         # "a photo of a"
        # print("ctx_init: ", ctx_init)
        dtype = clip_model.dtype
        ctx_dim = clip_model.ln_final.weight.shape[0]               # text encoder hidden size(512)
        # print("ctx_dim: ", ctx_dim)
        self.dim = clip_model.text_projection.shape[1]              # dim : 512
        # print("dim: ", self.dim)
        clip_imsize = clip_model.visual.input_resolution            # 224
        # print("clip_imsize: ", clip_imsize)
        cfg_imsize = cfg.INPUT.SIZE[0]                              # 224
        # print("After resizing clip_imsize: ", clip_imsize)
        ...
        if self.tp:
            if ctx_init and n_ctx <= 4:                             # use given words to initialize context vectors
                ...
                # print("ctx_init.replace: ", ctx_init)
                ...
                ctx_vectors = embedding[0, 1: 1 + n_ctx, :]         #torch.Size([2, 512])
    ...
    def forward(self):
        ...
        if self.proj != None:
            vctx = self.proj(self.ctx)                          # vctx: [2, 768]
        else:
            vctx = self.vctx
        ...
        prompts = self.construct_prompts(ctx, prefix, suffix)   # [31, 77, 512]
...
class CustomCLIP(Base_CustomCLIP):
    ...
    def forward(self, image, label=None, epoch=None, train=False, construct=False, source=True):
        ...
        # torch.save(text_features_u, "/data/dxw/PMCC/assets/VIT/text_features" + self.cfg.DATASET.NAME + ".pt")
        # sys.exit()
...
class PDA(BaseDA):
    def build_model(self):
        ...
        # with torch.no_grad():
        #     text_features_u = self.model.text_encoder_u(self.model.tokenized_prompts_u.to(self.model.logit_scale.device))
        #     text_features_u = text_features_u / text_features_u.norm(dim=-1, keepdim=True)
        #
        # torch.save(text_features_u,
        #            "/data/dxw/PMCC/assets/RN50/text_features" + cfg.DATASET.NAME + ".pt")
        # sys.exit()
    ...
    def construct_bank(self):
        ...
        #
        # torch.save(self.model.prompt_learner.source_feat_bank,
        #            "/data/dxw/PMCC/assets/" + self.cfg.DATASET.NAME + "/" + DOMAINS[source_domain] + "Centroid.pt")
        # sys.exit()
        ...
        #     DOMAINS = {'a': "amazon", 'w': "webcam", 'd': "dslr"}
        #
        # torch.save(self.model.prompt_learner.source_feat_bank, "/data/dxw/PMCC/assets/" + self.cfg.DATASET.NAME + "/" + DOMAINS[source_domain] +"Centroid_v14.pt")
        # torch.save(self.model.prompt_learner.target_feat_bank, "/data/dxw/PMCC/assets/" + self.cfg.DATASET.NAME + "/" + DOMAINS[target_domain] +"Centroid_v14.pt")
        # sys.exit()
    ...
    def T_SNE_combined(self):
        ...
        # source_domain, target_domain = domains.split('-')[0], domains.split('-')[1]
        #
        # source_feat_bank = torch.load("/data/dxw/PMCC/assets/" + self.cfg.DATASET.NAME + "/" + DOMAINS[source_domain] + "Centroid.pt")
        # target_feat_bank = torch.load("/data/dxw/PMCC/assets/" + self.cfg.DATASET.NAME + "/" + DOMAINS[target_domain] + "Centroid.pt")
        #
        source_bank = torch.mean(self.model.prompt_learner.source_feat_bank.reshape(self.n_cls, self.model.K, self.model.dim), dim=1)
        target_bank = torch.mean(self.model.prompt_learner.target_feat_bank.reshape(self.n_cls, self.model.K, self.model.dim), dim=1)
        ...
        for batch_idx, batch in enumerate(combined_loader):
            ...
            image_features = self.model.prompt_learner.attn_block(text_features, image_features, source_bank, target_bank)
        ...
        out_dir = "/data/dxw/PMCC/tsne/PDA/OfficeHome"

        plt.title('PDA' + ' (' + str(self.domains).upper() + ')', fontdict={"family": "Times New Roman", "size": 64})

        plt.savefig(out_dir + '/PDA-' + str(self.domains).upper() + '.pdf')
